...
import xserver.autoapi.constant.CachedConstant;
import xserver.autoapi.utils.FileUtil;
import xserver.autoapi.utils.PropertiesUtil;
import xserver.autoapi.utils.ConUtil;

/**
 * 填充信息到controller.html文件中
 */
public class FillInfo2Con {
...
//			getTerminalShells();
	...
	private static void getTerminalShells() {
		switch (ConfigConstant.mobile2Tv) {
		case "mobile":
			getMobileShells();
			break;
		case "tv":
	...
	public static void fillInfo(String name) {
		...
		// 依据类名获取到所有的接口地址
		interfaceNames = CachedConstant.contName2Interface.get(name);

		// 获取html文件的地址
		filePath = CachedConstant.controller2HtmlPath.get(name);

		// 获取类注释
		clazzMean = CachedConstant.contName2Means.get(name);
	 ...
	 * 创建shell
	 */
	private static void getMobileShells() {
		try {
			PropertiesUtil.getInstance().getPropertiesByClassPath("/mobile_url.sh");
		} catch (Exception e1) {
			log.error(e1.getMessage());
		}
		String path = System.getProperty("conf.dir");
		File f = new File(path + "/mobile_url.sh");
		if (f.exists()) {
			if (f.length() != 0) {
				f.delete();
		...
		try {
			...
			bw.newLine();
			bw.newLine();
			for (String url : CachedConstant.urls) {
				bw.write(
						"tac /letv/nginx_logs/access.log | awk -F'GET' '{print $2}' | awk -F'HTTP/' '{print $1}' | grep ");
				bw.write(url + " | tail -1 ");
				bw.write(" && ");
//				bw.newLine();
			}
	...
	private static void getTVShells() {
		try {
			PropertiesUtil.getInstance().getPropertiesByClassPath("/tv_url.sh");
		} catch (Exception e1) {
			log.error(e1.getMessage());
		}
		String path = System.getProperty("conf.dir");
		File f = new File(path + "/tv_url.sh");
		if (f.exists()) {
			if (f.length() != 0) {
				f.delete();
				f = new File(f.getAbsolutePath());
	...
	private static void getLIVEShells() {
		try {
			PropertiesUtil.getInstance().getPropertiesByClassPath("/live_url.sh");
		} catch (Exception e1) {
			log.error(e1.getMessage());
		}
		String path = System.getProperty("conf.dir");
		File f = new File(path + "/live_url.sh");
		if (f.exists()) {
			if (f.length() != 0) {
				f.delete();
				f = new File(f.getAbsolutePath());
		...
		try {
			...
			for (String url : CachedConstant.urls) {
				bw.write(
						"cat /letv/nginx_logs/access.log | awk -F'GET' '{print $2}' | awk -F'HTTP/' '{print $1}' | grep ");
				bw.write(url + " | tail -1 ");
				bw.newLine();
			}
			bw.flush();
	...
	private static void getOneInterFaceMsg(BufferedWriter bufferWritter, int i, String name) throws IOException {
		 ...
		 * 获取字段注释 name为接口信息
		 */
		Map<String, String> paraMap = CachedConstant.url2params.get(interfaceNames.get(i).toString());

		/**
		 * 获取接口对应的响应信息
		 */
		String responseInfo = CachedConstant.url2resposne.get(interfaceNames.get(i));
	...
	private static void getJSONResponse(BufferedWriter bufferWritter, int i) {
		try {
			...
			bufferWritter.write("<a href=\"javascript:;\"" + " data-url=\"" + url + "\"" + " class=\"openLayer\">"
					+ "JSON" + "</a>");
	...
	private static void getRequestExample(BufferedWriter bufferWritter, int i) throws IOException {
		switch (ConfigConstant.mobile2Tv) {
		case "mobile":
			getMobileRequestExample(bufferWritter, i);
			break;
		case "tv":
			getTvRequestExample(bufferWritter, i);
			break;
		case "live":
			getLiveRequestExample(bufferWritter, i);
			break;
		default:
			break;
		}
	...
	// TODO
	private static void getLiveRequestExample(BufferedWriter bufferWritter, int i) throws IOException {
		String rp = interfaceNames.get(i);
		if (rp != null) {
			Map<String, String> urlMap = FileUtil.getUrls();
	...
	private static void getTvRequestExample(BufferedWriter bufferWritter, int i) throws IOException {
		String rp = interfaceNames.get(i);
		if (rp != null) {
			Map<String, String> urlMap = null;
			if (FileUtil.getUrlMap() != null && FileUtil.getUrls().size() != 0) {
				urlMap = FileUtil.getUrlMap();
			} else {
				urlMap = FileUtil.getUrls();
			}

			if (urlMap != null && urlMap.size() != 0) {
				if (urlMap.get(rp) != null) {
					bufferWritter.write("<p><strong>请求示例</strong></p>");
					bufferWritter.write("<div class=\"blockquote\">");
					String url = "http://10.182.192.45" + urlMap.get(rp);
					bufferWritter.write("<a href=\"" + url + "\" target=\"_blank\">" + "example" + "</a>");
					bufferWritter.write("</div>");
				} else {
	...
	private static void getMobileRequestExample(BufferedWriter bufferWritter, int i) throws IOException {
		String rp = interfaceNames.get(i);
		if (rp != null) {
			Map<String, String> urlMap = FileUtil.getUrls();
			if (urlMap != null && urlMap.size() != 0) {
	...
	private static void getResponse(BufferedWriter bufferWritter, int i, String responseInfo) throws IOException {
		...
		bufferWritter.write("<div class=\"blockquote\">");
		String rp = CachedConstant.interface2ResponseHtml.get(interfaceNames.get(i));
		if (rp != null) {
			String substr = rp.substring(rp.indexOf(apidocs) + 8, rp.length());
			String rs = responseInfo.replace("<", CachedConstant.fis).replace(">", CachedConstant.end);
	...
	private static void getMeaning(BufferedWriter bufferWritter, int i) throws IOException {
		String interMeans = CachedConstant.url2name.get(interfaceNames.get(i));
		...
		String upath = "";
		if ("mobile".equalsIgnoreCase(ConfigConstant.mobile2Tv) || "live".equalsIgnoreCase(ConfigConstant.mobile2Tv)) {
