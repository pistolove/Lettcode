...
import xserver.autoapi.constant.FileConstant;
...
public class FileUtil {
	...
	public static Map<String, String> getUrls() {
		...
		case "mobile":
			return getMobileUrls(path + "/mobile_urls.txt");
		case "tv":
			return getTvUrls(path + "/tv_urls.txt");
		case "live":
			return getLiveUrsl(path + "/live_urls.txt");
		default:
			return null;
		}
	}

	private static Map<String, String> getLiveUrsl(String string) {
		return null;
	}

	private static Map<String, String> getTvUrls(String path) {
		try {
			...
			while ((temp = breader.readLine()) != null) {
				if (StringUtils.isNotBlank(temp)) {
					String u = temp.trim();
					if (u.indexOf(".") != -1) {
						if (u.contains("iptv") || u.contains("/location")) {
							String key = u.replace("/iptv/api/new", "").trim().replace("/iptv/api", "").trim();
							urlMap.put(key.substring(0, key.indexOf(".")), u.substring(0, u.length()-1));
						} else {
							urlMap.put(u.substring(0, u.indexOf(".")), u.substring(u.length()-1));
						}
					}
				}
			}
	...
	private static Map<String, String> getMobileUrls(String path) {
		try {
			FileReader freader = new FileReader(path);
			BufferedReader breader = new BufferedReader(freader);
			String temp = "";
