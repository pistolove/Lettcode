...
import jprotobuf194.HelloCons$$JProtoBufClass;
import jprotobuf194.HelloJson$$JProtoBufClass;
...
//            System.out.println("jprotobuf 1.9.4 序列化耗时："+ (etime_jpb_encode-stime_jpb_encode) + "ms;总大小："+jsonb.length);
//
//
...
//            System.out.println("jprotobuf 1.9.4 序列化耗时111："+ (etime_jpb_encode1-stime_jpb_encode1) + "ms;总大小："+jsonb1.length);
            ...
            System.out.println("jprotobuf 1.9.4 create 耗时："+ (etime_jpb_end-stime_jpb_create) + "ms");
            ...
            Long etime_jpb_encode = System.currentTimeMillis();
            System.out.println("jprotobuf 1.9.4 序列化耗时："+ (etime_jpb_encode-stime_jpb_encode) + "ms;总大小："+bytes.length);
...
//            System.out.println("jprotobuf 1.9.4 反序列化耗时："+ (etime_jpb_decode-stime_jpb_decode) + "ms");

            for(int i = 0;i<5;i++){
                Long stime_jpb_encode1 = System.currentTimeMillis();
                byte[] bytes1 = consCodec.encode(helloCons);
                Long etime_jpb_encode1 = System.currentTimeMillis();
                System.out.println("jprotobuf 1.9.4 序列化耗时："+ (etime_jpb_encode1-stime_jpb_encode1) + "ms;总大小："+bytes1.length);
...
//        System.out.println("jprotobuf 1.9.4 code:"+code);
    }
}
